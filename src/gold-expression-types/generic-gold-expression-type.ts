...
const regexp = (() => {
    ...
    const genericGoldSuffixExpParts = new RegExp(`(?:${goldBaseSuffixParts}){0,1}`, 'i');

    return new RegExp(`(?<=^)(?:${regexpParts.gold.source}${genericGoldSuffixExpParts.source})(?=$)`, 'i');
})();
...
export const GenericGoldExpressionType = GoldExpressionType.create(regexp, (expression: any, expressionType: GoldExpressionTypeMatchableResolvable<MatchGroups>) => {
    ...
    if (match) {
        ...
        if (match.gold) {
            ...
            totalCopper = new Decimal(`${match.neg ?? ""}${match.gold.replace(/[,_]/g, '')}.${match.fractionalGold ?? '0'}`).mul(multiplier);
